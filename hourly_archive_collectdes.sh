...
if [ -f "$BASE_DIR/preflight_collectdes.sh" ]; then
    $BASE_DIR/preflight_collectdes.sh
    ret=$?
    if [ $ret -ne 0 ]; then
        echo "[$(date)] Preflight check failed; aborting" >&2
        exit 2
    fi
fi
...
for host_fs in $FILE_SYSTEMS; do
    ...
    set -x
    /usr/bin/flock --nonblock "$SCHEDULE_FILE" "$ARCHIVE_COLLECTDES" \
        --output "$output_file" \
        $INIT_ARGS \
        --num-nodes 288 \
        --ssds-per-node 4 \
    ...
    if [ -d "${START_DATE}" ]; then
        ...
        echo "[$(date)] ERROR: $(basename $ARCHIVE_COLLECTDES) did not create $output_file" >&2
        global_failure=-1
        continue
    else
        echo "[$(date)] ERROR: $(basename $ARCHIVE_COLLECTDES) resulted in an unknown error" >&2
        global_failure=-2
        continue
    fi
    ...
    if [ "$ret" -eq 0 ]; then
        ...
        if /usr/bin/flock --nonblock "$SCHEDULE_FILE" /bin/true; then
            ...
            echo "[$(date)] ERROR: $(basename $ARCHIVE_COLLECTDES) returned code $ret" >&2
        else
            ### flock came back with an error, so the schedule is still locked
            echo "[$(date)] ERROR: Another instance of $(basename $ARCHIVE_COLLECTDES) is already running" >&2
        fi
